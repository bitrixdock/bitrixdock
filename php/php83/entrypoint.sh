...
if [ "$SECURITY_HARDENING" = "true" ]; then
    ...
    cp /tmp/php-security.ini /etc/php/8.3/fpm/conf.d/99-security.ini
    ...
    if [ "$DISABLE_DANGEROUS_FUNCTIONS" = "false" ]; then
        ...
        sed -i 's/^disable_functions.*/; disable_functions = /' /etc/php/8.3/fpm/conf.d/99-security.ini
    fi
fi
...
if [ "$DEVELOPMENT_MODE" = "false" ]; then
    ...
    echo "display_errors = Off" > /etc/php/8.3/fpm/conf.d/95-production.ini
    echo "display_startup_errors = Off" >> /etc/php/8.3/fpm/conf.d/95-production.ini
    echo "error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT" >> /etc/php/8.3/fpm/conf.d/95-production.ini
fi
