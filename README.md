...
## Сброс базы данных и сайта
После того, как bitrix-setup.php скачает данные для установки сайта, имеет смысл скопировать эти данные, чтобы потом заново не скачивать, когда нужно будет сбросить сайт.
Сброс базы данных:
```
docker ps
// смотрим id контейнера с mysql и подключаемся к нему:
sudo docker exec -i -t 6d2e8f5fa157 /bin/bash
// подключаемся к mysql
mysql -u root -p123 bitrix
// удаляем БД
drop database bitrix;
// создаем заново
