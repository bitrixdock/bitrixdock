...
### (optional) Определение имени домена

Чтобы обращаться к хосту по доменному имени, вы можете:
...
Также доступен SSL с самоподписанным сертификатом, который генерируется с помощью mkcert.
