...
Для включения контейнера cron используйте профиль при запуске:
```shell
docker compose -f docker-compose.secure.yml --profile cron up -d
```
Контейнер выполняет задания cron с ограниченными правами и отдельной конфигурацией безопасности.
