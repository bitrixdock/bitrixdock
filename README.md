...
### Защищенная конфигурация (опционально)

BitrixDock предоставляет дополнительные опции безопасности для production окружений. **Все функции безопасности отключены по умолчанию** для обеспечения обратной совместимости.

#### Быстрый старт с безопасностью

1. Используйте защищенный шаблон окружения:
   ```shell
   cp .env_template.secure .env
   ```

2. Настройте уровень безопасности в `.env`:
   ```dotenv
   SECURITY_HARDENING=true          # Включить усиленную безопасность
   DISABLE_DANGEROUS_FUNCTIONS=true # Отключить опасные PHP функции
   BLOCK_CRON_ACCESS=true          # Заблокировать доступ к cron для www-data
   DEVELOPMENT_MODE=false          # Отключить режим разработки для production
   ENABLE_CRON=true               # Добавить защищенный контейнер cron
   ```

3. Запустите с защищенной конфигурацией:
   ```shell
...
При включении `ENABLE_CRON=true` добавляется отдельный контейнер для выполнения заданий cron с ограниченными правами и отдельной конфигурацией безопасности.

#### Дополнительная информация

Подробное руководство по миграции и настройке безопасности: [SECURITY_MIGRATION.md](SECURITY_MIGRATION.md)

⚠️ **Важно**: Тщательно протестируйте все функции Битрикс после включения защиты, некоторые модули могут требовать определенных PHP функций.
